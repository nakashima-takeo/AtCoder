...
int main(void)
{
  ...
  for(ll i = 0LL; i < n; i++){
    ...
    rep(j, 11){
      ...
      result += safe_multiply(a[i] % mod, safe_multiply(pow_number, ten_ex_count[j]) % mod) % mod;
      result %= mod;
    }
    result = (result + safe_multiply(a[i] % mod, (i % mod) % mod) % mod);
  }
